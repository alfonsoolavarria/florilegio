# 🌸 Florilegio de la Fe

> **"Profundizando en la intimidad con Dios"**

¡Bienvenido a **Florilegio de la Fe**! 🌟 Este es un rincón digital diseñado para el alma inquieta y el corazón sediento de intimidad con Dios. No es solo un sitio web; es un florilegio (Del lat. flos, flōris 'flor' y legĕre 'escoger'. Colección de trozos selectos de materias literarias) de tesoros bíblicos, históricos y espirituales en una web moderna.

---

## ✨ ¿Cuál es la idea?

Florilegio nace de la pasión por rescatar la intimidad con Dios. Queremos que cada lectura sea una experiencia de cercanía con el Señor. Aquí exploramos:

*   📖 **La Palabra de Dios:** El mapa de nuestra existencia.
*   ⛪ **Historia de la Iglesia:** Crónicas de fe que nos conectan con nuestras raíces.
*   🕊️ **Vida de los Santos:** Héroes reales que nos demuestran que la santidad es un camino posible.

---

## 🎨 Un Diseño con "Alma"

Hemos puesto tanto amor en los bits como en las palabras:

*   **Efecto "Paper-Edge":** Un divisor orgánico que imita el borde de un papel antiguo, rompiendo la frialdad digital.
*   **Logo Dinámico:** Una bienvenida con estilo vertical-flip cada vez que entras al home.
*   **Estética Editorial:** Una paleta de colores crema y arena (#f7f3ed) que cuida tu vista y eleva el contenido.
*   **Buscador Minimalista:** Una barra expansiva sutil que aparece solo cuando la necesitas.

---

## 🚀 El Motor (Tech Stack)

Bajo este aspecto artesanal, hay tecnología de punta:

*   **Backend:** [Django](https://www.djangoproject.com/) (Python) - Robusto, seguro y listo para crecer.
*   **Frontend:** HTML5, JS y un **Sistema de Diseño CSS a medida** (Vanilla CSS para un control total del píxel).
*   **Interactividad:** Swiper para los contenidos destacados y micro-animaciones fluidas.

---

Hecho con ❤️ por [Alfonso Olavarria](https://github.com/alfonsoolavarria) y su copiloto de IA. 

*"Él es más satisfecho en nosotros, cuando nosotros estamos más satisfechos en Él."*
